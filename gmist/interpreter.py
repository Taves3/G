...
class Interpreter:
    ...
    def visit_Assign(self, node: Assign):
        ...
        if isinstance(bv, Float):
            ...
            match declared:
                ...
                case "list<int>":
                    # this is nightmarish
                    value = ListType([Integer(element) for element in bv.get_attribute("__iterate__").call(self, []).elements])
                case _:
    ...
    def visit_Declare(self, node: Declare):
        ...
        if isinstance(bv, Float):
            ...
            match node.kind:
                ...
                case "list<int>":
                    # NO!!!!!!!!!!!!
                    # THE CORE PHILOSOPHY FOR THIS IS THAT THIS KIND OF THING DOESN'T EXIST!!!!!!
                    # MAKE THE INCLUDE THINGY WORK AND THEN MAKE LIBRARIES AND PUT THIS IN A LIBRARY!!!!!
                    value = ListType([Integer(element) for element in bv.get_attribute("__iterate__").call(self, []).elements])
                case "function":
                    value = bv
                case _:
    ...
    def visit_ForLoop(self, node: ForLoop):
        if isinstance(node.condition, In):
            ...
            for element in self.run_func(target.get_attribute("__iterate__"), []).elements:
                self.frame.locals[name] = element
